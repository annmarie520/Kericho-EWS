...
####FIG 2: Rolling forecast p-values####
load("pvals_rf")
stat_names<-c("Variance", "First Difference of Variance", "Autocovariance", "Lag-1 Autocorrelation", "Decay Time", "Mean", "Index of Dispersion", "Coefficient of Variation", "Skewness", "Kurtosis")

par(mfrow=c(5,2))
for(i in 1:10){
  plot(sapply(pvals_rf, function(x) x[[i]]), xaxt='n', lwd=2, cex.main=2, cex.lab=1.5, cex.axis=1.2, xlab="Time to Transition (Months)", ylab=NA, ylim=c(0,.3), main=stat_names[i], type="l")
...
par(mfrow=c(5,2))
for(i in 1:10){
  plot(sapply(pvals_rf_spearman, function(x) x[[i]]), xaxt='n', lwd=2, cex.main=2, cex.lab=1.5, cex.axis=1.2, xlab="Time to Transition (Months)", ylab="p-value", ylim=c(0,.3), main=stat_names[i], type="l", col="blue", lty=3)
...
names(pval)<-names(pvals_tw[[1]][[1]])

for(beg in 1:11){
  for(end in 1:11){
    for(stat in names(pvals_tw[[1]][[1]])){
...
####SUPPLEMENTARY FIG S4: Line graphs of each rolling window statistic over time (top), null distributions of tau for each statistic with test value shaded (bottom)####
load("pvals")
load("cors")
load("indics")
kericho<-read.csv("kericho.csv")
...
par(mfrow=c(8, 3), mai = c(.5, 0.6, .5,.5))
...
for(i in 1:10){  
  plot(204:340, kericho$BBK[204:340]*.5, main=stat_names[i], type="h", cex.main=1.5, cex.axis=1.2, xlab=NA, ylim=c(0,100), ylab="Cases", xaxt='n', cex.lab=1.5)
...
for(i in 1:10){
  ...
  clr[bin]<-"red"
  plot(h, col=clr, main=stat_names[i], xlab=NA, ylab="Frequency", cex.lab=1.5, cex.axis=1.2, cex.main=1.5, cex.sub=1.5)
}
